...
def pq_fit(p, q, delta, lambd, absolute_value, filter_var):
    ...
    dqdp = linear.solve_lsq_filtered(
...
def analyze_hosting_capacity(
    ...
    for study_idx, (p, q, v) in enumerate(zip(P.T, Q.T, V_train.T)):
        ...
        v_k_max, q_k_max = (
            V_test[k_max, study_idx],
            q[k_max],
        )  # V and Q observed at the maximum voltage timepoint
...
def compute_hc(v_k_max, dvdp, dvdq, dqdp, pf, q_k, v_lim=1.05):
    ...
    qp_HC = (v_lim - v_k_max) / (
        dvdp + dvdq * dqdp
    )  # HC via active/reactive power sensitivities in tandem
    signed_hc = (v_lim - v_k_max) / (
        dvdp - np.sign(q_k) * dvdq * dqdp
    )  # HV via P/Q and switchin signs of complex plane
    pf_scaled_hc = (
        (v_lim - v_k_max) / (dvdp - np.sign(q_k) * dvdq * dqdp)
    ) / pf  # PF scaling using learned power factor
    arbitrary_pf_scaled_hc = (
        (v_lim - v_k_max) / (dvdp + dvdq * dqdp)
    ) / 0.7  # arbitrary 0.7 fixed pf injection
