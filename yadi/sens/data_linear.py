...
@jit
def excitation_filter(X,v,quantile,absolute_value=False,filter_var='p',return_idx_filtered=False):
    """
    ...
    for i,v_i in enumerate(V.T):
        ...
        if(not fit_offset):
            ...
            if(i==0): #At beginning, only contain upper tri diagonal
                ...
                coef_i = pinv(np.vstack([X[:,i-1].T,x_i.T,X[:,i+1].T,np.ones(len(x_i)).T]).T,lambd) @ v_i
                coef[:,i] = coef_i[:-1]
